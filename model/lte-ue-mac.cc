...
{
  NS_LOG_INFO (this << (uint32_t) params.lcid);
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_INFO("Reset UE " << m_rnti << " at time " << Simulator::Now().GetSeconds());
  m_uePhySapProvider->NotifyConnectionExpired();
      ...
      if (dci.m_ndi == 1)
        {
          ...
          NS_LOG_INFO (this << " RNTI " << m_rnti << ": UL-CQI notified TxOpportunity of " << dci.m_tbSize << " => " << bytesPerActiveLc << " bytes per active LC" << " statusPduMinSize " << statusPduMinSize << " at time " << Simulator::Now().GetSeconds());
      ...
      else
        {
          ...
          NS_LOG_INFO (this << " UE MAC RETX HARQ " << (uint16_t)m_harqProcessId << " rnti " << m_rnti);
