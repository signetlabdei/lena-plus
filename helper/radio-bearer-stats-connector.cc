...
{
  NS_LOG_FUNCTION (this << imsi << " cellID " << cellId << " rnti " << rnti << " ConnectSrb0Traces at time " << Simulator::Now().GetSeconds());
  ...
  if (m_pdcpStats)
    {
      ...
      Config::Disconnect (ueManagerPath + "/Srb1/LtePdcp/RxPDU",
                          MakeBoundCallback (&UlRxPduCallback, arg));
      Config::Disconnect (ueManagerPath + "/Srb1/LtePdcp/TxPDU",
                          MakeBoundCallback (&DlTxPduCallback, arg));
...
{
  NS_LOG_FUNCTION (this << imsi << cellId << rnti << " ConnectSrb1TracesUe Ue at time " << Simulator::Now().GetSeconds());
   if (m_rlcStats)
    {
      Ptr<BoundCallbackArgument> arg = Create<BoundCallbackArgument> ();
      arg->imsi = imsi;
  ...
  if (m_pdcpStats)
    {
      ...
      Config::Disconnect (ueRrcPath + "/Srb1/LtePdcp/RxPDU",
           MakeBoundCallback (&DlRxPduCallback, arg));
      Config::Disconnect (ueRrcPath + "/Srb1/LtePdcp/TxPDU",
           MakeBoundCallback (&UlTxPduCallback, arg));
...
{
  NS_LOG_FUNCTION (this << context << " ConnectTracesUeIfFirstTime Ue at time " << Simulator::Now().GetSeconds());
...
{
  NS_LOG_FUNCTION (this << context << " ConnectTracesEnbIfFirstTime enb at time " << Simulator::Now().GetSeconds());
...
{
  NS_LOG_FUNCTION (this << context << " ConnectTracesUe at time " << Simulator::Now().GetSeconds());
...
{
  NS_LOG_FUNCTION (this << context << " ConnectTracesEnb at time " << Simulator::Now().GetSeconds());
...
{
  NS_LOG_FUNCTION (this << context << " DisConnectTracesUe at time " << Simulator::Now().GetSeconds());
  if (m_imsiSeenUe.find (imsi) != m_imsiSeenUe.end ())
    {
      NS_LOG_LOGIC (this << "expected context should match /NodeList/*/DeviceList/*/LteUeRrc/");
      std::string basePath = context.substr (0, context.rfind ("/"));
...
{
  NS_LOG_FUNCTION (this << context << " DisConnectTracesEnb at time " << Simulator::Now().GetSeconds());
  if (m_imsiSeenEnb.find (imsi) != m_imsiSeenEnb.end ())
    {
      NS_LOG_LOGIC (this << "expected context  should match /NodeList/*/DeviceList/*/LteEnbRrc/");
      std::ostringstream basePath;
